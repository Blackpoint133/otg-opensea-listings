...
export async function verifyContinuityLossRebaselineDurable(
  ...
  try {
    ...
    for (const row of linked.rows) {
      ...
      if (!baseline || row.initial_baseline_adoption_id !== plan.adoptionId || row.protocol_address !== plan.protocolAddress || canonicalEvidence(row.raw_baseline_listing) !== canonicalEvidence(baseline.rawBaselineListing)) return false;
    }

...
function candidate(classes: Array<"PRESENT" | "ABSENT_CANDIDATE" | "BLOCKED">) {
  ...
  const localOrders = classes.map((classification, index) => ({ orderHash: hashes[index], status: classification === "BLOCKED" ? "sold" as const : "active" as const, isActive: classification !== "BLOCKED", needsReconciliation: classification === "BLOCKED", lastOrderEventTimestamp: "2026-08-23T12:00:00.000Z", lastOrderEventVersion: "1" }));
...
async function writeLegacyGraph(writer: any, candidatePayload: unknown, generationPayload: unknown) {
  ...
  const candidateEnvelope = { artifactType: "candidate-bundle", artifactId: "candidate-bundle", artifactSchemaVersion: "candidate-bundle-envelope-v1", sweepId: SWEEP, candidateModelVersion: OFFLINE_CANDIDATE_MODEL_VERSION, sourceProvenance: PROVENANCE, candidateBundleHash: sha256Canonical(canonicalCandidatePayload), authorityGranted: false, payload: canonicalCandidatePayload };
  const candidateRef = await writer.writeArtifact({ artifactType: "candidate-bundle", artifactId: "candidate-bundle", relativePath: "candidate-bundle.json", schemaVersion: "candidate-bundle-envelope-v1", payload: candidateEnvelope });
  ...
  const barrierRef = await writer.writeArtifact({ artifactType: "barrier-evaluation", artifactId: "barrier-evaluation", relativePath: "barrier-evaluation.json", schemaVersion: "barrier-evaluation-envelope-v1", payload: barrierEnvelope });

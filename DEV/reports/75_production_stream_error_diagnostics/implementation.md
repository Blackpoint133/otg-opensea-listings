...
String redaction now removes credentials embedded in key/value text, including whitespace, colon/equal, camelCase, underscore, hyphen, and query-parameter forms. Authorization Bearer/Basic credentials are removed as a whole value. The same redaction applies to extracted Error messages and stacks and to nested string fields. Symbol-keyed values are inspected using the same secret-key policy; secret-like symbol values are replaced with `<REDACTED>` while non-secret symbol diagnostics remain bounded and available.

...
export interface ContinuityLossRebaselineDependencies {
  ...
  readonly generation: () => Promise<RebaselineGenerationOutput>;
...
export async function runContinuityLossRebaseline(deps: ContinuityLossRebaselineDependencies): Promise<ContinuityLossRebaselineResult> {
  ...
  try { verified = await (deps.verify ?? (async () => true))(plan, generated.publication); if (!verified) reason = "POST_ADOPTION_DURABLE_VERIFICATION_FAILED"; } catch { reason = "POST_ADOPTION_DURABLE_VERIFICATION_FAILED"; }

...
async function writeLegacyGraph(writer: any, candidatePayload: unknown, generationPayload: unknown) {
  ...
  const candidateEnvelope = { artifactType: "candidate-bundle", artifactId: "candidate-bundle", artifactSchemaVersion: "candidate-bundle-envelope-v2", sweepId: SWEEP, candidateModelVersion: OFFLINE_CANDIDATE_MODEL_VERSION, sourceProvenance: PROVENANCE, candidateBundleHash: sha256Canonical(canonicalCandidatePayload), authorityGranted: false, payload: canonicalCandidatePayload };
  const candidateRef = await writer.writeArtifact({ artifactType: "candidate-bundle", artifactId: "candidate-bundle", relativePath: "candidate-bundle.json", schemaVersion: "candidate-bundle-envelope-v2", payload: candidateEnvelope });

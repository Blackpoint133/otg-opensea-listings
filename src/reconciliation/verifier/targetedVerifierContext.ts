...
export function deriveTargetedVerifierContext(evidence: IntegratedEvidenceResult, orderHash: string, metadata: TargetedVerifierContextMetadata): TargetedVerifierContext {
  ...
  if (!evidence.manifest || evidence.manifest.sweepId !== payload.sweepId || evidence.barrier.candidateArtifactHash !== evidence.candidateRef.contentHash || !evidence.manifest.artifacts.some((ref) => ref.contentHash === evidence.barrierRef?.contentHash)) throw new Error("EVIDENCE_GRAPH_MISMATCH");

...
test("adapter preserves numeric type before item-type schema checks", () => {
  ...
  for (const token of ['"2"', '2.0', '2e0', '-0', '2147483648', '{"kind":"number","raw":"2"}']) {
    const bytes = new TextEncoder().encode(source.replace('"item_type":2', '"item_type":' + token));
...
test("adapter preserves numeric type before order-type and string identity checks", () => {
  ...
  for (const token of ['"0"', '0.0', '0e0', '-0', '{"kind":"number","raw":"0"}']) {
    assert.notEqual(adaptOpenSeaExactOrder(input(new TextEncoder().encode(source.replace('"order_type":0', '"order_type":' + token)))).outcome, "VALID", token);
  }
  for (const field of ['"identifier":"1"', '"identifier_or_criteria":"1"', '"start_amount":"1"', '"end_amount":"1"']) {
...
const duplicateMutations: [string, string, string][] = [
  ...
  ["offer.identifier", '"identifier_or_criteria":"1"', '"identifier_or_criteria":"2","identifier_or_criteria":"1"'],
...
test("same order hash with different trusted token provenance cannot reuse an observation", async () => {
  ...
  try {
    ...
    const source = new TextDecoder().decode(raw())
      ...
      .replace('"identifier_or_criteria":"1"', '"identifier_or_criteria":"2"');

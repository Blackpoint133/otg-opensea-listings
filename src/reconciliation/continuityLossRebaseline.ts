...
export async function captureRecoveryEntryAnchor(pool: DbPool, supersededPublicationId: string, supersededPublicationSequence: number, supersededSweepId: string, capturedAt = new Date().toISOString()): Promise<RecoveryEntryAnchor> {
  ...
  const accepted = await client.query<any>("SELECT generation_publication_id,publication_sequence,sweep_id,scope,publication_state FROM public.targeted_verifier_generation_publications WHERE publication_state='ACCEPTED' ORDER BY publication_sequence DESC");

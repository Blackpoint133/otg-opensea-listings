...
function anchorMaterial(value: Omit<RecoveryEntryAnchor, "commitment">): unknown {
  return { schemaVersion: value.schemaVersion, database: value.database, schema: value.schema, capturedAt: value.capturedAt, journalHighWater: value.journalHighWater, supersededPublicationId: value.supersededPublicationId, supersededPublicationSequence: value.supersededPublicationSequence, supersededSweepId: value.supersededSweepId, adoptionCount: value.adoptionCount, listingCount: value.listingCount, activeListingCount: value.activeListingCount, genericPendingCount: value.genericPendingCount, specializedPendingCount: value.specializedPendingCount, processingCount: value.processingCount, failedCount: value.failedCount, staleProcessingCount: value.staleProcessingCount, validProductionIngestionLeaseCount: value.validProductionIngestionLeaseCount };
}
...
export function createRecoveryEntryAnchor(input: RecoveryEntryAnchorInput): RecoveryEntryAnchor {
  ...
  if (!input.supersededSweepId) fail("CONTINUITY_LOSS_RECOVERY_ANCHOR_INVALID");
...
export async function captureRecoveryEntryAnchor(pool: DbPool, supersededPublicationId: string, supersededPublicationSequence: number, supersededSweepId: string, capturedAt = new Date().toISOString()): Promise<RecoveryEntryAnchor> {
  const client = await pool.connect();
  try {
  await client.query("BEGIN ISOLATION LEVEL REPEATABLE READ READ ONLY");
  const identity = await client.query<any>("SELECT current_database() AS database,current_schema() AS schema");
  if (identity.rows[0]?.database !== "server_otg" || identity.rows[0]?.schema !== "public") fail("CONTINUITY_LOSS_RECOVERY_DATABASE_MISMATCH");
  const publication = await client.query<any>("SELECT generation_publication_id,publication_sequence,publication_state,sweep_id,scope FROM public.targeted_verifier_generation_publications WHERE generation_publication_id=$1", [supersededPublicationId]);
  if (publication.rows.length !== 1 || Number(publication.rows[0].publication_sequence) !== supersededPublicationSequence || publication.rows[0].publication_state !== "ACCEPTED" || publication.rows[0].sweep_id !== supersededSweepId) fail("CONTINUITY_LOSS_RECOVERY_PUBLICATION_BINDING_INVALID");
  const scope: any = parseJson(publication.rows[0].scope); if (!record(scope) || scope.chain !== SUPPORTED_CHAIN || scope.collectionSlug !== SUPPORTED_COLLECTION_SLUG || scope.contractAddress !== SUPPORTED_CONTRACT_ADDRESS) fail("CONTINUITY_LOSS_RECOVERY_PUBLICATION_BINDING_INVALID");
  const accepted = await client.query<any>("SELECT generation_publication_id,publication_sequence,sweep_id,scope,publication_state FROM public.targeted_verifier_generation_publications WHERE publication_state='ACCEPTED' ORDER BY publication_sequence DESC FOR UPDATE");
  const scoped = accepted.rows.filter((p: any) => { const s = parseJson(p.scope); return record(s) && s.chain === SUPPORTED_CHAIN && s.collectionSlug === SUPPORTED_COLLECTION_SLUG && s.contractAddress === SUPPORTED_CONTRACT_ADDRESS && s.protocolAddress === scope.protocolAddress; });
  const maxSeq = scoped.reduce((m: number, p: any) => Math.max(m, Number(p.publication_sequence)), -1); if (scoped.filter((p: any) => Number(p.publication_sequence) === maxSeq).length !== 1 || maxSeq !== supersededPublicationSequence) fail("CONTINUITY_LOSS_RECOVERY_PUBLICATION_NOT_CURRENT");
  const journal = await client.query<any>("SELECT event_id::text AS event_id,received_at::text AS received_at FROM public.opensea_listings_events_v2 ORDER BY event_id DESC LIMIT 1");
  const journalRow = journal.rows[0] ?? (await client.query<any>("SELECT transaction_timestamp()::text AS received_at")).rows[0];
  const listing = await client.query<any>("SELECT count(*)::text AS listing_count,count(*) FILTER (WHERE is_active=true)::text AS active_count FROM public.opensea_listings_v2");
  const inbox = await client.query<any>("SELECT count(*) FILTER (WHERE processing_status='pending' AND COALESCE(raw_payload->'payload'->'rest_backfill_source'->>'source','') <> 'opensea_rest_events_backfill')::text AS generic_pending,count(*) FILTER (WHERE processing_status='pending' AND COALESCE(raw_payload->'payload'->'rest_backfill_source'->>'source','') = 'opensea_rest_events_backfill')::text AS specialized_pending,count(*) FILTER (WHERE processing_status='processing')::text AS processing,count(*) FILTER (WHERE processing_status='failed')::text AS failed,count(*) FILTER (WHERE processing_status='processing' AND processing_started_at IS NOT NULL AND processing_started_at <= now()-interval '15 minutes')::text AS stale FROM public.opensea_listings_events_v2");
  const adoption = await client.query<any>("SELECT count(*)::text AS count FROM public.opensea_listings_initial_baseline_adoptions WHERE generation_publication_id=$1", [supersededPublicationId]);
  const locks = await client.query<any>("SELECT a.pid::text AS pid,l.classid::text AS classid,l.objid::text AS objid,l.objsubid,l.mode,l.granted,a.application_name,a.datname FROM pg_locks l JOIN pg_stat_activity a ON a.pid=l.pid WHERE l.locktype='advisory' AND l.objsubid=1 AND l.granted=true AND l.mode='ExclusiveLock' AND a.datname='server_otg' AND a.application_name='opensea_listings_v2_production_ingestion'");
  ...
  const result = createRecoveryEntryAnchor({ capturedAt, journalHighWater: { eventId: String(journalRow?.event_id ?? "0"), receivedAt: String(journalRow?.received_at ?? capturedAt) }, supersededPublicationId, supersededPublicationSequence, supersededSweepId, adoptionCount: Number(adoption.rows[0]?.count ?? 0), listingCount: Number(l.listing_count ?? 0), activeListingCount: Number(l.active_count ?? 0), genericPendingCount: Number(i.generic_pending ?? 0), processingCount: Number(i.processing ?? 0), failedCount: Number(i.failed ?? 0), staleProcessingCount: Number(i.stale ?? 0), validProductionIngestionLeaseCount: validLeases, specializedPendingCount: Number(i.specialized_pending ?? 0) });
...
export interface ContinuityLossRebaselinePlanV2 extends Omit<InitialBaselineAdoptionPlanV1, "schemaVersion"|"adoptionId"> {
  ...
  readonly supersededPublicationSequence: number;
  readonly supersededSweepId: string;
  readonly recoveryEntryJournalEventId: string;
  readonly recoveryEntryJournalReceivedAt: string;
...
export function createContinuityLossRebaselinePlan(input: { evidence: unknown; projection: InitialGenerationBaselineProjection; integratedEvidence: IntegratedEvidenceResult; publication: GenerationPublicationEvidenceV1; supersededPublicationId: string; supersededPublicationSequence: number; supersededSweepId: string; recoveryEntryAnchor: RecoveryEntryAnchor }): ContinuityLossRebaselinePlanV2 {
  ...
  if (input.recoveryEntryAnchor.supersededPublicationId !== input.supersededPublicationId || input.recoveryEntryAnchor.supersededPublicationSequence !== input.supersededPublicationSequence || input.recoveryEntryAnchor.supersededSweepId !== input.supersededSweepId) fail("CONTINUITY_LOSS_RECOVERY_ANCHOR_BINDING_MISMATCH");
  ...
  const supersededSweepId = input.supersededSweepId;
  const sweepMaterial = { supersededSweepId };
  const rowsCommitment = sha256Canonical({ schema: PLAN_SCHEMA, recoveryMode: CONTINUITY_LOSS_REBASELINE, supersededPublicationId: input.supersededPublicationId, supersededPublicationSequence: input.supersededPublicationSequence, ...sweepMaterial, generationPublicationId: input.publication.generationPublicationId, generationPublicationSequence: input.publication.publicationSequence, sourceEvidenceHash: v1.sourceEvidenceHash, snapshotArtifactHash: v1.snapshotArtifactHash, generationRootHash: v1.generationRootHash, candidateArtifactHash: v1.candidateArtifactHash, barrierArtifactHash: v1.barrierArtifactHash, scope: v1.scope, scopeFingerprint: v1.scopeFingerprint, protocolAddress: v1.protocolAddress, rows: v1.rows.map((r) => ({ ...r, rawBaselineListingHash: sha256Canonical(r.rawBaselineListing) })) });
  const recoveryEntryJournalEventId = input.recoveryEntryAnchor.journalHighWater.eventId;
  const recoveryEntryJournalReceivedAt = input.recoveryEntryAnchor.journalHighWater.receivedAt;
  const adoptionId = sha256Canonical({ schema: PLAN_SCHEMA, recoveryMode: CONTINUITY_LOSS_REBASELINE, supersededPublicationId: input.supersededPublicationId, supersededPublicationSequence: input.supersededPublicationSequence, ...sweepMaterial, generationPublicationId: input.publication.generationPublicationId, generationPublicationSequence: input.publication.publicationSequence, recoveryEntryAnchorCommitment: input.recoveryEntryAnchor.commitment, recoveryEntryJournalEventId, recoveryEntryJournalReceivedAt, stableWatermark: v1.stableWatermark, snapshotCompletedAt: v1.snapshotCompletedAt, rowsCommitment });
  const plan = deepFreeze({ ...v1, schemaVersion: PLAN_SCHEMA, recoveryMode: CONTINUITY_LOSS_REBASELINE, adoptionId, supersededPublicationId: input.supersededPublicationId, supersededPublicationSequence: input.supersededPublicationSequence, ...sweepMaterial, recoveryEntryJournalEventId, recoveryEntryJournalReceivedAt, recoveryEntryAnchorCommitment: input.recoveryEntryAnchor.commitment, rowsCommitment });
...
export function continuityRebaselineInsertContract(): { readonly targetColumnCount: number; readonly expressionCount: number; readonly parameterPositions: Readonly<Record<string, number>> } { return { targetColumnCount: CONTINUITY_REBASELINE_INSERT_COLUMNS.length, expressionCount: CONTINUITY_REBASELINE_INSERT_EXPRESSIONS.length, parameterPositions: Object.freeze({ source: 15, adoptedAt: 16, rawLastEvent: 17, protocolAddress: 18, adoptionId: 19, rawBaselineListing: 20 }) }; }
function baselineValues(row: InitialBaselineRowPlan, adoptedAt: string, adoptionId: string): unknown[] { return [row.orderHash,row.nftId,row.chain,row.contractAddress,row.tokenId,row.collectionSlug,row.sellerAddress,row.priceRaw,row.priceNormalized,row.paymentTokenAddress,row.paymentTokenSymbol,row.paymentTokenDecimals,row.listingStartAt,row.expirationAt,SOURCE,adoptedAt,JSON.stringify(null),row.protocolAddress,adoptionId,JSON.stringify(row.rawBaselineListing)]; }
...
async function replay(client: TransactionClient, plan: ContinuityLossRebaselinePlanV2, adoptedAt: string): Promise<void> {
  ...
  for (const row of events.rows) {
    ...
    const businessTime = eventTime(normalized); if (businessTime === null) fail("CONTINUITY_LOSS_REBASELINE_EVENT_TIME_UNPROVEN");
    const provenBusinessTime = businessTime as number; if (provenBusinessTime <= Date.parse(plan.snapshotCompletedAt)) continue;
  ...
  async adopt(plan: ContinuityLossRebaselinePlanV2): Promise<{ outcome: "ADOPTED"|"ALREADY_ADOPTED"; adoptionId: string; adoptedOrderCount: number }> {
    ...
    try {
      ...
      if (existingReceipt.rows.length > 0) {
        if (existingReceipt.rows.length !== 1 || !validateExistingReceipt(existingReceipt.rows[0], plan)) fail("CONTINUITY_LOSS_REBASELINE_DURABLE_CORRUPTION");
        const linked = await client.query<any>("SELECT order_hash,initial_baseline_adoption_id,protocol_address,raw_baseline_listing FROM public.opensea_listings_v2 WHERE initial_baseline_adoption_id=$1", [plan.adoptionId]);
        const expected = new Map(plan.rows.map((r) => [r.orderHash, r]));
        if (linked.rows.length !== plan.expectedOrderCount || new Set(linked.rows.map((r: any) => r.order_hash)).size !== plan.expectedOrderCount || linked.rows.some((r: any) => { const e = expected.get(r.order_hash); return !e || r.initial_baseline_adoption_id !== plan.adoptionId || r.protocol_address !== plan.protocolAddress || canonicalEvidence(parseJson(r.raw_baseline_listing)) !== canonicalEvidence(e.rawBaselineListing); })) fail("CONTINUITY_LOSS_REBASELINE_DURABLE_CORRUPTION");
        await client.query("COMMIT"); return { outcome: "ALREADY_ADOPTED", adoptionId: plan.adoptionId, adoptedOrderCount: plan.expectedOrderCount };
      }
      ...
      const superseded = await client.query<any>("SELECT generation_publication_id,publication_sequence,publication_state,sweep_id FROM public.targeted_verifier_generation_publications WHERE generation_publication_id=$1 FOR UPDATE", [plan.supersededPublicationId]);
      if (superseded.rows.length !== 1 || Number(superseded.rows[0].publication_sequence) !== plan.supersededPublicationSequence || superseded.rows[0].publication_state !== "ACCEPTED" || superseded.rows[0].sweep_id !== plan.supersededSweepId) fail("CONTINUITY_LOSS_REBASELINE_SUPERSEDED_PUBLICATION_INVALID");
      const supersededAdoption = await client.query<any>("SELECT count(*)::int AS count FROM public.opensea_listings_initial_baseline_adoptions WHERE generation_publication_id=$1", [plan.supersededPublicationId]);
      if (Number(supersededAdoption.rows[0]?.count ?? 0) !== 0) fail("CONTINUITY_LOSS_REBASELINE_SUPERSEDED_PUBLICATION_ALREADY_ADOPTED");
